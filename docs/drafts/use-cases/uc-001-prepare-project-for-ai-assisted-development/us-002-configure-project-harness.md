...
The user follows the project-configuration guide to replace relevant scaffold completion markers with project-specific decisions. Existing information is preserved unless the user confirms a change, while unavailable values remain explicit and the setup can be resumed later.

## User Interaction

### Trigger

The user asks to complete or resume project-specific scaffold configuration.

### User-Provided Information

- Testing strategy, glossary terms, README details, conventions, quality commands, and shared test-support decisions.

### System Response

- Updates approved project-level files, preserves existing information unless the user confirms a change, and reports any completion markers that remain.

### Failure Signals

- A requested value is not known, an update would overwrite existing information without confirmation, or permission is unavailable.
...
- **When**: the user resumes project configuration, answers the targeted questions for the identified gaps, and confirms any project-specific defaults or revisions
...
- **Given**: the initialized scaffold exists with project-specific completion markers and project configuration has selected one or more target files to configure
- **When**: the setup flow is interrupted or fails before every selected value is recorded
- **Then**: the system SHALL preserve the initialized scaffold and any completed updates, leave remaining completion markers visible, and allow the user to resume project configuration to continue from the remaining gaps

### Scenario 3: Project Information Is Not Available Yet

- **Given**: project configuration reaches a project-level value that is not established in conversation and the user cannot provide it
...
- **Then**: the system SHALL explain which files were updated, created, and still need attention, and direct a greenfield project toward the requirement-capture workflow while offering a brownfield project the documentation-reconstruction workflow
