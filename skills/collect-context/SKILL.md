...
3. When the task materially interprets, creates, updates, or reviews Project Knowledge, read the active Knowledge Schema before that work. Do not require Schema loading for a purely mechanical repository operation. Read relevant effective Project Rules and the relevant Workflow before changing behavior or durable knowledge.
4. Start from the current Workflow Phase Goal and Required Outcome. Locate only the relevant knowledge records, applicable Project Rules, implementation, tests, repository instructions, and external evidence. Do not load all project knowledge by default.
