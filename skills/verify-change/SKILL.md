...
1. Read the relevant effective Project Rules, applicable Governance constraints, the relevant Requirement, its owned Acceptance Criteria, relevant Verification Items, and the changed files.
2. Identify the smallest verification that can demonstrate each changed behavior and each Verification Item's bounded evidence scope. Treat external dependencies as assumed inputs or controlled substitutes when they are outside the boundary; do not substitute an interaction being verified. Increase coverage for higher-risk behavior, interfaces, migrations, security-sensitive code, or irreversible effects.
